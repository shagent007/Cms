...
public class OrderController : ControllerBase
{
    ...
    private DbSet<Entities.Order> DbSet => _dataContext.Set<Entities.Order>();
    public OrderController(DataContext dataContext)
    {
        _dataContext = dataContext;
    }

    private IQueryable<Entities.Order> FilterPredicate(Filter filter, IQueryable<Entities.Order> items)
    {
        switch (filter.Name)
        {
    ...
    public async Task<IActionResult> GetList(string?filter, int? pageIndex, int?pageSize, string? orderField, string?orderType)
    {
        try
        {
            ...
            return Ok(new ListResponse<Entities.Order>(items, total));
        }
        catch (Exception e)
        {
    ...
    [HttpPost]
    public async Task<IActionResult> Add([FromBody] Entities.Order model)
    {
        try
        {
            DbSet.Add(model);
    ...
    [HttpPut]
    public async Task<IActionResult> Update(Entities.Order model)
    {
        try
        {
            DbSet.Update(model);
